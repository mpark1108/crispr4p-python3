"""Command-line boundary for CRISPR4P."""
...
from .service import (
    Crispr4pService,
    OligoLengthError,
    normalize_oligo_query,
)


def build_parser():
    """Build the existing CRISPR4P command-line interface."""
...
def format_design_result(result):
    """Format a structured design result using the legacy terminal layout."""
    output = io.StringIO()
...
def format_oligo_result(
    result,
    include_query_header=True,
    include_loading_messages=True,
):
    """Format a structured oligo result using the legacy terminal layout."""
    output = io.StringIO()
...
def run_cli(arguments, service, output=None):
    """Parse one CLI request, call the service, and render its result."""
    if output is None:
        output = sys.stdout
    ...
    if args.oligo:
        try:
            normalized_sequence, seed, pam = normalize_oligo_query(
                args.oligo
            )
    ...
    output.write(format_design_result(result))
    return 0


def main(arguments=None):
    """Run the standalone CLI and retain its legacy runtime report."""
