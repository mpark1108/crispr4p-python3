...
from crispr4p.guides import build_suffix_match_table, sequences_match
...
class TestGuideMatching(unittest.TestCase):
    ...
    def test_exact_matching_preserves_legacy_cumulative_counts(self):
        returned_guide, table = build_suffix_match_table(
            self.guide,
            self.index,
            n_mismatch=0,
        )
    ...
    def test_one_mismatch_is_allowed_only_after_exact_eight_base_lookup(self):
        _, table = build_suffix_match_table(
            self.guide,
            self.index,
            n_mismatch=1,
        )
    ...
    def test_sequence_comparison_preserves_mismatch_thresholds(self):
        self.assertFalse(sequences_match("AAAA", "AAAT", -1))
        self.assertFalse(sequences_match("AAAA", "AAAT", 0))
        self.assertTrue(sequences_match("AAAA", "AAAT", 1))
        self.assertTrue(sequences_match("AAAA", "AAAT", 2))
    ...
    def test_legacy_primer_design_methods_delegate_to_public_functions(self):
        ...
        with patch(
            "crispr4p.crispr4p.build_suffix_match_table",
